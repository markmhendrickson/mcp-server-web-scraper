            ...
            try:
                if scrape_method == "playwright":
                    ...
                    scraped_data = scrape_with_apify(
                        url, credentials.get("apify_token")
                    )
                    method_used = "apify"
                    break
        ...
        if not scraped_data:
            return [
                TextContent(
                    type="text",
                    text=json.dumps(
                        {"error": "All scraping methods failed", "details": errors},
                        indent=2,
                    ),
                )
            ]
    ...
    try:
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps(
                    {"error": f"Error listing conversations: {str(e)}"}, indent=2
                ),
            )
        ]
...
async def handle_get_details(args: dict) -> list[TextContent]:
    ...
    try:
        ...
        sorted_nodes = sorted(
            mapping.items(), key=lambda x: int(x[0].split("_")[1]) if "_" in x[0] else 0
        )
        ...
        return [
            TextContent(
                type="text",
                text=json.dumps(
                    {"error": f"Error getting conversation details: {str(e)}"}, indent=2
                ),
            )
        ]
