                ...
                try:
                    ...
                    try:
                        await page.wait_for_selector(
                            "article, main, h1, body", timeout=10000
                        )
                    except:
                        pass
                    ...
                    debug_html = (
                        page_html[:10000] if len(page_html) > 10000 else page_html
                    )
                    ...
                    for selector in transcript_selectors:
                        try:
                            ...
                            if transcript_elem:
                                ...
                                if (
                                    transcript and len(transcript) > 100
                                ):  # Ensure it's substantial
                                    break
                        except:
                            continue
                    ...
                    if not transcript:
                        try:
                            transcript_button = await page.query_selector(
                                'button:has-text("Transcript"), a:has-text("Transcript"), [aria-label*="transcript" i]'
                            )
                            if transcript_button:
                                ...
                                for selector in transcript_selectors:
                                    transcript_elem = await page.query_selector(
                                        selector
                                    )
                                    if transcript_elem:
                                        transcript = await transcript_elem.inner_text()
                    ...
                    if not transcript:
                        try:
                            article_body = await page.query_selector(
                                'article, [role="article"], main, [class*="article"]'
                            )
                            if article_body:
                    ...
                    if not transcript:
                        try:
                            ...
                            main_content = await page.query_selector(
                                'main, article, [role="main"], [class*="article"]'
                            )
                            if main_content:
                    ...
                    return {
                        ...
                        "page_text_preview": (
                            page_text[:1000] if page_text else None
