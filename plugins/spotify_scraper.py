                        ...
                        if (
                            ...
                            try:
                                api_responses.append(
                                    {"url": url_resp, "data": response.json()}
                                )
                            except Exception:
                                pass
                    except Exception:
            ...
            try:
                    ...
                    for selector in count_selectors:
                        ...
                        if elem:
                            ...
                            if match:
                                ...
                                print(
                                    f"  Found total track count on page: {total_tracks_from_page}"
                                )
                                break
                ...
                while scroll_attempts < max_scroll_attempts:
                    ...
                    if tracklist_container:
                        ...
                        current_scroll = tracklist_container.evaluate(
                            "element => element.scrollTop"
                        )
                        scroll_height = tracklist_container.evaluate(
                            "element => element.scrollHeight"
                        )
                        client_height = tracklist_container.evaluate(
                            "element => element.clientHeight"
                        )

                        # Scroll down by viewport height each time
                        new_scroll = min(current_scroll + client_height, scroll_height)
                        tracklist_container.evaluate(
                            f"element => element.scrollTop = {new_scroll}"
                        )
                    ...
                    if current_track_count == last_track_count:
                        ...
                        if no_change_count >= 5:
                            ...
                            print(
                                f"  No new tracks after {no_change_count} attempts, stopping"
                            )
                            break
                    else:
                        no_change_count = 0
                ...
                print(
                    f"Finished scrolling after {scroll_attempts} attempts, found {last_track_count} tracks"
                )
                        ...
                        for key in ["playlist", "playlists", "content", "data"]:
                            if key in data and isinstance(data[key], dict):
                                ...
                                if "tracks" in nested or "items" in nested:
                                    ...
                                    tracks_in_resp = (
                                        nested.get("tracks", {}).get("items", [])
                                        if isinstance(nested.get("tracks"), dict)
                                        else []
                                    )
                                    if not tracks_in_resp:
                ...
                if all_tracks_from_api and playlist_data:
                    ...
                    for track_item in all_tracks_from_api:
                        ...
                        track_id = (
                            track_obj.get("id") if isinstance(track_obj, dict) else None
                        )
                ...
                if not playlist_data:
                    ...
                    try:
                        page.wait_for_selector(
                            'div[data-testid="entityTitle"], h1', timeout=10000
                        )
                    except Exception:
                        pass
                    ...
                    title_elem = page.query_selector(
                        'div[data-testid="entityTitle"], h1'
                    )
                    playlist_title = (
                        title_elem.inner_text() if title_elem else "Unknown Playlist"
                    )
                    ...
                    try:
                        ...
                        for row in track_rows:
                            try:
                                ...
                                artist_elems = row.query_selector_all(
                                    'a[href*="/artist/"]'
                                )
                                if artist_elems:
                                    artists = [
                                    ...
                                    artist_text = row.query_selector(
                                        'span:has-text("•")'
                                    )
                                    if artist_text:
                                        artists = [
                                            artist_text.inner_text()
                                            .strip()
                                            .replace("•", "")
                                            .strip()
                                        ]
                                ...
                                if track_link:
                                    ...
                                    if track_url and not track_url.startswith("http"):
                                        track_url = (
                                            f"https://open.spotify.com{track_url}"
                                        )

                                # Extract duration
                                duration = None
                                duration_elem = row.query_selector(
                                    'span[data-testid="duration"]'
                                )
                                if duration_elem:
                                    duration = duration_elem.inner_text().strip()

                                if track_name != "Unknown Track" or artists != [
                                    "Unknown Artist"
                                ]:
                                    tracks.append(
                                        {
                ...
                current_visible_tracks = (
                    last_track_count
                    if "last_track_count" in locals()
                    else (len(all_tracks_from_api) if all_tracks_from_api else 25)
                )
                if (
                    total_tracks_from_page
                    and total_tracks_from_page > current_visible_tracks
                ):
                    print(
                        f"  Page shows {total_tracks_from_page} tracks, but we only have {current_visible_tracks}. Continuing to scroll..."
                    )
                    ...
                    while (
                        additional_scrolls < max_additional
                        and current_visible_tracks < total_tracks_from_page
                    ):
                        if tracklist_container:
                            ...
                            new_scroll = min(
                                current_scroll + scroll_increment, scroll_height
                            )
                            tracklist_container.evaluate(
                                f"element => element.scrollTop = {new_scroll}"
                            )
                        ...
                        track_rows = page.query_selector_all(
                            'div[data-testid="tracklist-row"]'
                        )
                        ...
                        if new_count > current_visible_tracks:
                            ...
                            if additional_scrolls % 15 == 0:
                                print(
                                    f"    Progress: {current_visible_tracks}/{total_tracks_from_page} tracks loaded..."
                                )
                            ...
                            if no_progress_count >= 10:
                                ...
                                if tracklist_container:
                                    ...
                                    if new_count > current_visible_tracks:
                                        ...
                                        no_progress_count = 0
                                        print(
                                            f"    Jumped to bottom, found {current_visible_tracks} tracks"
                                        )
                                    else:
                    ...
                    print(
                        f"  Additional scrolling complete after {additional_scrolls} attempts, now have {current_visible_tracks} visible tracks"
                    )
                ...
                if playlist_data:
                    ...
                    if isinstance(owner_data, dict):
                        owner = owner_data.get(
                            "display_name", owner_data.get("id", owner)
                        )
                ...
                playlist_title = (
                    playlist_title if playlist_title else "Unknown Playlist"
                )

                # Extract description (if available)
                description_elem = page.query_selector(
                    '[data-testid="playlist-description"]'
                )
                description = (
                    description_elem.inner_text() if description_elem else None
                )
                ...
                if total_tracks_from_page and len(tracks) < total_tracks_from_page:
                    print(
                        f"  Re-extracting from DOM after scrolling (have {len(tracks)}, need {total_tracks_from_page})..."
                    )
                    ...
                    if len(track_rows) > len(tracks):
                        print(
                            f"  Found {len(track_rows)} tracks in DOM, re-extracting..."
                        )
                        ...
                        for row in track_rows:
                            try:
                                # Extract track name
                                track_name = "Unknown Track"
                                name_elem = row.query_selector(
                                    'div[data-testid="entityTitle"] a, a[href*="/track/"]'
                                )
                                if name_elem:
                                    track_name = name_elem.inner_text().strip()
