                ...
                try:
                    ...
                    try:
                        await page.wait_for_selector(
                            'section:has-text("Transcript"), article', timeout=10000
                        )
                    except:
                        pass
                    ...
                    try:
                        ...
                        transcript_heading = await page.query_selector(
                            'h2:has-text("Transcript"), h3:has-text("Transcript"), [role="heading"]:has-text("Transcript")'
                        )
                            ...
                            try:
                                ...
                                if title_elem:
                                    ...
                                    if (
                                        ...
                                        and len(title) > 5
                                        and title.lower()
                                        not in ["metacast.app", "metacast", "podcast"]
                                    ):
                                        break
                            except:
                                continue
                    ...
                    try:
                        ...
                        print(
                            f"DEBUG: Page title from browser: {page_title_actual}",
                            file=sys.stderr,
                        )
                        print(
                            f"DEBUG: Page text length: {len(page_text) if page_text else 0}",
                            file=sys.stderr,
                        )
                        print(
                            f"DEBUG: Page text preview (first 1000 chars): {page_text[:1000] if page_text else 'None'}",
                            file=sys.stderr,
                        )
                    except:
                        pass
                        ...
                        try:
                            ...
                            for section in sections:
                                try:
                                    ...
                                    heading = await section.query_selector(
                                        'h2, h3, [role="heading"]'
                                    )
                                    if heading:
                                        ...
                                        if "transcript" in heading_text.lower():
                                            ...
                                            article = await section.query_selector(
                                                "article"
                                            )
                                            if article:
                    ...
                    if not transcript:
                        try:
                            main_content = await page.query_selector(
                                'main, article, [role="main"], [class*="content"], [class*="episode"]'
                            )
                            if main_content:
                            ...
                            for elem in all_text_elements:
                                try:
                                    ...
                                    if (
                                        not elem_text
                                        or len(elem_text) < 50
                                        or elem_text in seen_texts
                                    ):
                                        continue

                                    # Check if it looks like transcript content
                                    has_dialogue = ":" in elem_text or "?" in elem_text
                                    has_speaker = re.search(
                                        r"[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*:", elem_text
                                    )
                                    # Not navigation/UI
                                    is_not_ui = not any(
                                        ui_word in elem_text.lower()
                                        for ui_word in [
                                            "subscribe",
                                            "log in",
                                            "cookie",
                                    ...
                                    if len(elem_text) > 100 and is_not_ui:
                                        ...
                                        if has_dialogue and (
                                            has_speaker or len(elem_text) > 200
                                        ):
                    ...
                    if not transcript:
                        try:
                            ...
                            for details in details_elements:
                                ...
                                if (
                                    "transcript" in details_text.lower()
                                    and len(details_text) > 500
                                ):
                                    # Try to expand it
                                    try:
                                        await details.click()
                    ...
                    for selector in audio_selectors:
                        try:
                            ...
                            if audio_elem:
                                audio_url = await audio_elem.get_attribute(
                                    "src"
                                ) or await audio_elem.get_attribute("href")
                                if audio_url:
                                    break
                        except:
                            continue
                    ...
                    if not transcript and page_text:
                        try:
                            ...
                            if transcript_start > 0:
                                ...
                                transcript_lines = []
                                for line in lines_after_transcript[
                                    1:
                                ]:  # Skip the "Transcript" heading itself
                                    line = line.strip()
                                    if line:
                            ...
                            print(
                                f"DEBUG: Final transcript extraction error: {e}",
                                file=sys.stderr,
                            )
                            pass
