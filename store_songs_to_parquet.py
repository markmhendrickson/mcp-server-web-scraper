...
async def main():
    ...
    playlist_file = (
        ...
        / "imports"
        / "spotify"
        / "playlist_3i0FY58V7n8XIUyWDm3AYA.json"
    )
    ...
    for i, track in enumerate(tracks, 1):
        try:
            ...
            result = await call_parquet_tool(
                "upsert_record",
                {"data_type": "songs", "filters": filters, "record": song_record},
            )

            if result.get("error"):
                print(
                    f"  Error storing track {i} ({track.get('name', 'Unknown')}): {result.get('error')}"
                )
                error_count += 1
            else:
            ...
            print(
                f"  Exception storing track {i} ({track.get('name', 'Unknown')}): {e}"
            )
            error_count += 1
